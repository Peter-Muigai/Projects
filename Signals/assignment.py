...
def plot_frequency(x, Fs, title, filename=None):
    N = len(x)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("msg_time_domain.pdf")
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("am_time_domain.pdf")
plt.show()

plt.figure(figsize=(10, 4))
plt.plot(t, nbfm)
plt.title("NBFM Signal")
plt.xlabel("Time (s)")
plt.grid(True)
plt.tight_layout()
plt.savefig("nbfm_time_domain.pdf")
plt.show()

plt.figure(figsize=(10, 4))
plt.plot(t, wbfm)
plt.title("WBFM Signal")
plt.xlabel("Time (s)")
plt.grid(True)
plt.tight_layout()
plt.savefig("wbfm_time_domain.pdf")
plt.show()


plot_frequency(msg, Fs, "Message Spectrum", "msg_frequency_domain.pdf")
plot_frequency(am, Fs, "AM Spectrum", "am_frequency_domain.pdf")
plot_frequency(nbfm, Fs, "NBFM Spectrum", "nbfm_frequency_domain.pdf")
plot_frequency(wbfm, Fs, "WBFM Spectrum", "wbfm_frequency_domain.pdf")
...
# ---------------------------------------------------------
# FREQUENCY-DOMAIN ANALYSIS — RF INPUT (COMMON REFERENCE)
# ---------------------------------------------------------

plot_frequency(
    rx,
    Fs,
    "Received RF Signal Spectrum",
    "rf_input_spectrum.pdf"
)

# ---------------------------------------------------------
# TRF RECEIVER — FREQUENCY DOMAIN (NO MIXING)
# ---------------------------------------------------------

# TRF RF Bandpass Filtering (Selectivity entirely at RF)
trf_rf_out = bandpass_filter(rx, Fc - B / 2, Fc + B / 2, Fs)

plot_frequency(
    trf_rf_out,
    Fs,
    "TRF Receiver: RF Bandpass Filter Output",
    "trf_rf_filtered_spectrum.pdf"
)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("trf_time_domain.pdf")
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("superhet_time_domain.pdf")
plt.show()
...
plot_frequency(rf_out, Fs, "After RF Bandpass Filter", "rf_bpf.pdf")

# Mixer (Down-conversion)
lo = np.cos(2 * np.pi * (Fc - IF) * t)
mixed = rf_out * lo
plot_frequency(mixed, Fs, "After Mixing (RF → IF)", "mixing.pdf")

# IF Filter
if_out = bandpass_filter(mixed, IF - B / 2, IF + B / 2, Fs)
plot_frequency(if_out, Fs, "After IF Filter", "if_filter.pdf")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("superhet_recovered_audio.pdf")
plt.show()

plot_frequency(demod_out, Fs, "Recovered Audio Spectrum", "superhet_recovered_spectrum.pdf")
...
for key, values in results.items():
    ...
    plt.tight_layout()
    plt.savefig(f"{key}_snr_comparison.pdf")
    plt.show()
